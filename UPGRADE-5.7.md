...
* Changed the `plugin.xsd` so unknown elements in a `plugin.xml` are tolerated again, as they were up to 5.7.19.
  The known children of `<plugin>` are now declared as global elements and matched by a lax wildcard, because a
  wildcard next to named elements violates the "Unique Particle Attribution" constraint that `libxml2` >= 2.12
  enforces. Known elements are still fully validated.
